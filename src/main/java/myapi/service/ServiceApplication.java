...
@ComponentScan(excludeFilters = {
    @ComponentScan.Filter(
        type = FilterType.ASSIGNABLE_TYPE, classes = OpenAiEmbeddingClient.class
    ),
    @ComponentScan.Filter(
        type = FilterType.ASSIGNABLE_TYPE, classes = OllamaChatClient.class
    )
})
public class ServiceApplication {
    ...
    @Bean
    VectorStore vectorStore(@Qualifier("ollamaEmbeddingClient") EmbeddingClient ec,
                            JdbcTemplate t) {
    ...
    ApplicationRunner applicationRunner(
        ...
        return args -> {
            var exist = jdbcTemplate.queryForObject("SELECT EXISTS(select 1 from vector_store)",
                Boolean.class);
            if (Boolean.FALSE.equals(exist)) {
                init(vectorStore, jdbcTemplate, resource);
            }
            var response = chatbot.chat("what should I know about the transition to consumer " +
                                        "direct care network washington?");
...
class Chatbot {
    ...
    Chatbot(@Qualifier("openAiChatClient") ChatClient aiClient, VectorStore vectorStore) {
